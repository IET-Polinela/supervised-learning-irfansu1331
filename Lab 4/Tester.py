...
# Load Dataset
df = pd.read_csv('dataset_encoded.csv')  # Sesuaikan dengan lokasi dataset

# Membuat folder untuk menyimpan gambar hasil visualisasi
...
def visualize_outliers(df):
    ...
    filepath = os.path.join(save_dir, "outliers_boxplot.png")
    plt.savefig(filepath, dpi=300)
    plt.close()
    print(f"Gambar outliers disimpan di: {filepath}")
...
def plot_results(Y_test, Y_pred, model_name, filename):
    ...
    plt.savefig(filepath, dpi=300)
    plt.close()
    print(f"Gambar {model_name} disimpan di: {filepath}")
